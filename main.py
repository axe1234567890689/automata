...
x = int(screen_size[0] / zoom)
while x > 0:
    terrain[x] = {}
    terrains[x] = {}
    y = int(screen_size[1] / zoom)
    while y > 0:
        terrain[x][y] = -1
        terrains[x][y] = -1
        y -= 1
    ...
    for x in terrains:
        for y in terrains[x]:
            if terrains[x][y] == -2:
                terrain[x][y] = -1
                terrains[x][y] = -1
                pygame.draw.rect(screen, (0, 0, 0), [x * zoom, y * zoom, zoom, zoom])
            elif not terrain[x][y] == terrains[x][y] or terrain[x][y] == 6:
